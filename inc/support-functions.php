...
function travelo_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'travelo' ) . '</span></li>';
...
// Blog categories
function travelo_featured_post_cat(){
...
// social media
if ( ! function_exists( 'travelo_social' ) ) {
	function travelo_social( $args = array() ) {
		...
		$url = travelo_opt( 'travelo_social_url' );
...
// header cart count
function travelo_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( TRAVELO_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'travelo' ); ?>">
...
function travelo_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'travelo_contact7_form_content', 10, 2 );