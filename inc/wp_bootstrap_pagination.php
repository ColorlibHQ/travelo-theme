...
function travelo_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'travelo' ),
        'next_string'     => esc_html__( '&raquo;', 'travelo' ),
        'before_output'   => '<div class="travelo-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'travelo_pagination_defaults', $defaults )
    );
